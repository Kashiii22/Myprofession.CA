...
const navItems = [
  ...
  { label: "Mentor Requests", icon: FaChalkboardTeacher, path: "/superadmin/mentor-requests" },
  { label: "User Management", icon: FaUser, path: "/superadmin/users" },
  { label: "Transactions", icon: FaHandHoldingUsd, path: "/superadmin/transactions" },
  { label: "Analytics", icon: FaChartPie, path: "/superadmin/analytics" },
...
export default function SuperAdminSidebar({ className = "" }) {
  ...
  return (
    <>
      ...
      {/* Sidebar (Desktop) */}
      <aside className={`hidden md:flex md:flex-col w-64 bg-black border-r border-blue-900/40 p-4 ${className}`}>
        <div className="flex items-center gap-2 mb-8 pb-4 border-b border-blue-800">
          <FaUserShield className="text-blue-500 text-2xl" />
          <h2 className="text-white text-xl font-bold">SuperAdmin</h2>
        </div>
